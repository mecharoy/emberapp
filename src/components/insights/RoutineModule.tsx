    ...
    <ModuleCard title="Daily routine">
      {s.latest && (
        <div className="mb-4 flex flex-wrap items-end gap-6">
          <p className="max-w-md font-serif text-[18px] leading-snug text-fg">
      ...
      <table className="w-full text-[13.5px]">
        <tbody className="divide-y divide-line">
          {s.anchors.map((a) => (
            <tr key={a.id}>
              <td className="py-2 pr-2 text-fg" title={`On time = within ${HIT_WINDOW_MIN} minutes of your usual`}>{a.label}</td>
              <td className="py-2 tabular-nums text-fg-dim">{a.usual ?? "–"}</td>
              <td className="py-2 text-right text-fg-faint">
      ...
      {s.finding && (
        <button
          ...
          className="-mx-2 mt-3 w-full rounded-md px-2 py-2 text-left font-serif text-[16px] leading-snug text-fg transition-colors hover:bg-surface-high"
        >
          {s.finding.sentence}
        </button>
      )}
