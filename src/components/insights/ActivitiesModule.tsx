...
function Dots({ value, label }: { value: number | null; label: string }) {
  ...
  return (
    <span className="inline-flex gap-0.5" title={`${label}: ${value.toFixed(1)} of 3`} aria-label={`${label} ${value.toFixed(1)} of 3`}>
      {[1, 2, 3].map((n) => (
        <span key={n} className={`h-2 w-2 rounded-full ${n <= filled ? "bg-ember" : "bg-rule"}`} />
      ))}
    </span>
  );
}
...
export default function ActivitiesModule({
  ...
  const stats = activityStats(rows).slice(0, 12);
      ...
      {stats.length === 0 ? (
        ...
        <table className="w-full text-[13px]">
          ...
          <tbody className="divide-y divide-rule">
            {stats.map((a) => (
              <tr key={a.key} className="cursor-pointer hover:bg-paper-deep" onClick={() => onOpen(`activity: ${a.key}`, a.dates)}>
                <td className="py-2 text-ink">{a.key}</td>
                <td className="py-2 tabular-nums text-ink-soft">{a.days}</td>
                <td className="py-2">
