...
export default function WhatsNew({ screen }: { screen: FirstRunScreen | null }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center sm:items-center">
      <button aria-label="Close" onClick={close} className="fade-in absolute inset-0 bg-fg/30" />
      <div
        role="dialog"
        aria-label={`What's new in Elytra ${card.version}`}
        className="sheet-up relative flex max-h-[85%] w-full max-w-md flex-col rounded-t-2xl border border-line bg-surface shadow-[0_-8px_30px_-12px_rgba(40,35,30,0.35)] sm:rounded-2xl"
      >
        <div className="px-6 pb-3 pt-6">
          <p className="text-[12.5px] uppercase tracking-[0.14em] text-moss">Updated to {card.version}</p>
          <h2 className="mt-1 font-serif text-[24px] leading-tight text-fg">What&rsquo;s new</h2>
        </div>
        <ul className="flex flex-col gap-4 overflow-y-auto px-6 pb-4">
          {card.items.map((h) => (
            <li key={h.title} className="flex gap-3">
              ...
              <div>
                <p className="text-[15px] font-medium text-fg">{h.title}</p>
                <p className="mt-0.5 text-[14px] leading-relaxed text-fg-dim">{h.text}</p>
              </div>
            </li>
          ))}
        </ul>
